...
--run_once("urxvtd")
run_once("compton -cGfb -O 200 -I 200 -o 0.75 -r 10 -m 1")
--run_once("compton")
-- }}}
...
-- Textclock
mytextclock = awful.widget.textclock("<span font='ohsnap 5'> </span>%a %b %d %H:%M ")
...
volume = lain.widgets.alsabar({width = 55, ticks_size = 6, timeout = 1,
settings = function()
...
colors = 
{
    background = beautiful.bg_pbar,
    mute = "#EB8F8F",
    unmute = beautiful.fg_normal
}})
volmargin = wibox.layout.margin(volume.bar, 2, 10)
volmargin:set_top(8)
...
volicon:buttons (awful.util.table.join (
      ...
      awful.button ({}, 3, function()
        awful.util.spawn(string.format("amixer set %s toggle", volume.channel))
        volume.notify()
      end),
      awful.button ({}, 4, function()
        awful.util.spawn(string.format("amixer set %s %s+", volume.channel, volume.step))
        volume.notify()
      end),
...
small_spr = wibox.widget.textbox('<span font="ohsnap 4"> </span>')
bar_spr = wibox.widget.textbox('<span font="ohsnap 3"> </span>' .. markup("#333333", "|") .. '<span font="ohsnap 3"> </span>')
